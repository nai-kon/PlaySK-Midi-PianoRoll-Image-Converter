...
class DuoArtOrganSetting:
    def __init__(self, parent: ctk.CTk):
        ...
        for row_pos, val in self.midi_settings.items():
            ...
            section = ctk.CTkLabel(self.dialog, text=row_pos, font=font)
            section.grid(row=row_no, column=0, columnspan=4, padx=5, pady=(20, 10))
            row_no += 1

            headers = ["Hole No", "Name", "MIDI CH", "MIDI Note No"]
            for col, text in enumerate(headers):
                label = ctk.CTkLabel(self.dialog, text=text)
                label.grid(row=row_no, column=col, padx=5, pady=5)
            row_no += 1
            ...
            for hole_no, [hole_name, midi_ch, midi_no, _, _] in val.items():    
                ...
                self.midi_settings[row_pos][hole_no][3] = tmp

                # Note Number
                tmp = ctk.CTkComboBox(self.dialog, values=[str(i) for i in range(128)], width=80)
                tmp.set(str(midi_no))
                tmp.grid(row=row_no, column=3, padx=5, pady=2)
                self.midi_settings[row_pos][hole_no][4] = tmp
    ...
    def debug(self):
        ...
        # 列の重み付けを設定
        # for i, weight in enumerate([1, 2, 1, 1]):
        #     self.dialog.grid_columnconfigure(i, weight=weight)
